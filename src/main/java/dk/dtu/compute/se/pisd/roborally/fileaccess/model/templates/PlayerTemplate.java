...
public class PlayerTemplate {
    ...
    public PlayerTemplate fromPlayer(Player player)
    {
        ...
        program = new CommandCardFieldTemplate[player.getProgram().length];
        for (int i = 0;i<player.getProgram().length;i++)
        {
            program[i] = new CommandCardFieldTemplate().fromCommandCardField(player.getProgramField(i));

        }
        cards = new CommandCardFieldTemplate[player.getCards().length];
        for (int i = 0;i<player.getCards().length;i++)
        {
            cards[i] = new CommandCardFieldTemplate().fromCommandCardField(player.getCardField(i));
